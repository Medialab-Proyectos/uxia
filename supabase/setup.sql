...
-- 3d-sexies) Trazabilidad del vencimiento: cuando se cambia `due_date`, se conserva la fecha
-- ANTERIOR al cambio como soporte (queda a la vista en la tarjeta: "antes: dd/mm/aaaa") y el
-- MOTIVO del cambio (obligatorio en un popup antes de aceptar el nuevo vencimiento).
alter table tasks add column if not exists prev_due_date date;
alter table tasks add column if not exists due_change_reason text;
...
-- Índice GENERAL de proyectos (0..100) = "Satisfacción empresarial" del panel CEO de MediaLab.
-- NO es solo el rating (lo que dicen los clientes): combina satisfacción declarada (0.40),
-- cumplimiento/predictibilidad de entregas a tiempo (0.35) y salud de flujo — sin vencidas ni
-- bloqueos (0.25); si un componente no tiene datos, su peso se redistribuye. Ver detalle en
-- supabase/migration-indice-proyectos.sql. El portal MediaLab lee `indice` con SERVICE KEY.
...
select
  ...
  round((c_sat  * 100))::int as satisfaccion, round((c_cump * 100))::int as cumplimiento,
  round((c_flujo * 100))::int as flujo, activas, vencidas, bloqueadas, calificadas
from w;
