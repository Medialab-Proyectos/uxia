-- Migración: permitir al EMPLEADO marcar un Change Request como resuelto en SUS tareas.
-- Seguro e idempotente. Pégalo en Supabase → SQL Editor → Run.
-- (Ya está incluido en setup.sql; este archivo es solo el delta: quita la protección de
--  change_requests en el trigger para que el empleado pueda resolver un CR al re-enviar a
--  revisión. El admin conserva la última palabra, aprobando o volviendo a pedir cambios.)
...
begin
  if app_is_employee() then
    ...
    -- change_requests NO se revierte: el empleado puede marcar un CR como resuelto.
